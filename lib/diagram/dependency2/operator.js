...
class Dependency2DiagramOperator extends Dependency2DiagramBuilder {
  ...
  clearDependencyLines(lineClass) {
    ...
    this.rootSVGGroupSelection.selectAll(`circle${selector}`).classed(lineClass, false)
    this.rootSVGGroupSelection.selectAll(`text${selector}`).classed(lineClass, false)
  }
  ...
  _setDependencyLineEndpoint(nodeData, lineClass) {
    this.rootSVGGroupSelection.select(`circle[id='${nodeData.path}']`).classed(lineClass, true)
    this.rootSVGGroupSelection.select(`text[id='${nodeData.path}-lb']`).classed(lineClass, true)
  }
  ...
  _clearDependencyLineEndpointVisibility(originPath) {
    const targetNodePath = this.typeOfPath(originPath) === 'tp' ? this.parentPathOf(originPath) : originPath
    const resetTermPointDetector = (d) => d.type === 'tp' && d.visible && !this.matchChildPath(targetNodePath, d.path)
