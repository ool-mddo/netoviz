...
class Dependency2DiagramBuilder extends MultilayerDiagramBase {
  ...
  _makeNodeDataListOfEachNetworks() {
    ...
    for (const networkData of this.topologyData) {
      ...
      for (const node of networkData.nodes) {
        ...
        // append tps in node
        const termPoints = networkData.tps.filter((d) => this.matchChildPath(node.path, d.path))
        if (termPoints) {
   ...
   */
  _makeNodeCircles() {
    const updatedEntries = this.rootSVGGroupSelection.selectAll('circle.dep2').data(this._allVisibleNodes())
   ...
   */
  _makeNodeLabels() {
    const updatedEntries = this.rootSVGGroupSelection.selectAll('text.dep2').data(this._allVisibleNodes())
  ...
  _makeDependencyLineGroup() {
     ...
     * @type {Selection}
     */
    this.dependencyLineGroupSelection = this.rootSVGGroupSelection.append('g').attr('id', 'dep-lines')
  }
