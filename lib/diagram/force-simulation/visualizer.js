...
class ForceSimulationDiagramVisualizer extends BaseContainer {
  ...
  _drawNetworkDiagrams() {
    ...
    for (const networkData of this.topologyData) {
      ...
      const networkDiagram = new ForceSimulationDiagramOperator(networkData, callback)
      this.positionCache.loadToTopologyData(this.storageKey, networkData, networkDiagram)
