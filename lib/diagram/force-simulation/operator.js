...
class ForceSimulationDiagramOperator extends ForceSimulationDiagramSimulator {
   ...
   */
  _clearSelectReady() {
    selectAll('div#visualizer').selectAll('.select-ready').classed('select-ready', false)
  }
  ...
  _clickHandler(nodeData) {
    ...
    this.delayedClickCallback = timeout(() => this._clickOperation(nodeData), 300)
  }
  ...
  _setDiagramElementsHandler() {
    ...
    for (const target of targets) {
      this.rootSVGGroupSelection
        ...
        .call(drag().on('start', dragStart).on('drag', dragged).on('end', dragFinished))
    }
  }
  ...
  _appendTermPointInfoTable(nodeData) {
    ...
    this._refreshTermPointInfoTable(parentNode.parents.filter((p) => this.typeOfPath(p) === 'tp'))
  }
