...
class DependencyDiagramOperator extends DependencyDiagramBuilder {
  ...
  _makeDependencyLines(lines, lineClass) {
    ...
    this.dependencyLinesGroupSelection
      ...
      .data(lines.filter((line) => line.src.type === line.dst.type).map((line) => this._lineConverter(line)))
      .enter()
      .append('path')
  ...
  _pathsFromPairs(elementData, pairs) {
    ...
    return this.sortUniq(this.flatten([elementData.path, pairs.map((d) => d.src.path), pairs.map((d) => d.dst.path)]))
  }
  ...
  clickHandler(elementData) {
    ...
    this._runParentsAndChildren(elementData, makeSelectDepLines, setHighlightByPath)
  }
  ...
  _mouseOverHandler(event, elementData) {
    ...
    this._runParentsAndChildren(elementData, makeSelectReadyDepLines, setSelectReadyByPath)
  ...
  _mouseOutHandler(elementData) {
    ...
    this._runParentsAndChildren(elementData, clearSelectReadyDepLines, unsetSelectReadyByPath)
   ...
   */
  _setRootSVGZoom() {
    // ignore empty nodes case
    const lastNodes = this.topologyData.map((network) => network.nodes[network.nodes.length - 1]).filter((n) => n)
  ...
  setAllDiagramElementsHandler() {
    ...
    this.zoom = zoom()
      .scaleExtent([1 / 4, 5])
      .on('zoom', (event) => this.rootSVGGroupSelection.attr('transform', event.transform))
