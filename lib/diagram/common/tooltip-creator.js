...
import RfcL2TermPointAttribute from '../../../server/graph/rfc-model/term-point-attr-rfc-l2'
import RfcL3TermPointAttribute from '../../../server/graph/rfc-model/term-point-attr-rfc-l3'
...
} from '../../../server/graph/rfc-model/term-point-attr-mddo'
...
class TooltipCreator {
  ...
  enableTooltip(event, node, tooltipAnchorClickCallback) {
    ...
    // tooltip body
    console.log('[enableTooltip]: node class: ', node.constructor.name)
    if (node && Object.keys(node.attribute).length > 0) {
      console.log(`[enableTooltip: node attr class = ${node.attribute.class}`)
