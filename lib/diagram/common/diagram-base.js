...
class DiagramBase extends BaseContainer {
  ...
  makeDiagramContainer() {
    ...
    return select('body').select('div#visualizer').append('div').attr('class', 'network-layer')
  }
  ...
  makeRootSVG(svgId, svgClass, svgGroupId) {
    ...
    this.rootSVGSelection = this.diagramContainerSelection
      ...
      .attr('class', svgClass)
    /** @type {Selection} */
    this.rootSVGGroupSelection = this.rootSVGSelection.append('g').attr('id', svgGroupId)
  }
  ...
  _classOfDiffState(nodeData) {
    ...
    console.warn(`object ${nodeData.type}: ${nodeData.path} does not have diffState`)
    return ''
  }
  ...
  classStringFrom(elementData, classString) {
    ...
    const nodeBaseClasses = [diffState, this._classOfInactive(diffState), this._classOfAggregated(elementData)]
  ...
  controlButtonMouseOverCallback(selector) {
    return () => this.rootSVGSelection.select(selector).classed('select-ready', true)
  }
  ...
  controlButtonMouseOutCallback(selector) {
    return () => this.rootSVGSelection.select(selector).classed('select-ready', false)
  }
  ...
  _toggleActiveDiff() {
    ...
    visualizer.selectAll(`.${this.currentInactive}`).classed('inactive', false).classed('active', true)
    this.currentInactive = this.currentInactive === 'deleted' ? 'added' : 'deleted'
    visualizer.selectAll(`.${this.currentInactive}`).classed('inactive', true).classed('active', false)
  }
