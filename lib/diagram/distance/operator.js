...
class DistanceDiagramOperator extends DistanceDiagramBuilder {
  ...
  _setInitialZoom() {
    ...
    this.zoom.translateTo(this.rootSVGSelection, 0, 0, [this.width / 2, this.height / 2])
  }
  ...
  _findLinksConnectedWithNode(path) {
    const hasEndNodeByPath = (l) => l.sourceNodePath === path || l.targetNodePath === path
  ...
  _linkMouseOverHandler(linkData) {
    ...
    this._findNodesAtLinkEnd(linkData).forEach((n) => this._selectReadyNodeCircle(n))
  }
