...
class DistanceDiagramBuilder extends MultilayerDiagramBase {
   ...
   */
  _makeLinkSVGGroup() {
    this.linkGroupSelection = this.rootSVGGroupSelection.append('g').attr('id', 'links')
  }
   ...
   */
  _makeTpTpLinkLine() {
    this._makeLinkLine(this.wholeNodes, this.topologyData.links, 'inter-nodes', (d, classStr) =>
      this.classStringFrom(d, classStr)
    )
  }
  ...
  _makeSupportNodeLinks() {
    this._makeLinkLine(this.wholeNodes, this.topologyData.supportLinks, 'support-node', (_d, classStr) => classStr)
  }
  ...
  makeAllDiagramElements(topologyData) {
     ...
     */
    this.wholeNodes = topologyData.layouts.reduce((sum, l) => sum.concat(l.nodes), [])
