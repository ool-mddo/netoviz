...
class NestedDiagramOperator extends NestedDiagramBuilder {
  ...
  _findTargetRootNodes(i, j) {
    return this.topologyData.nodes.filter((node) => {
      ...
      return node.grid && ((i >= 0 && node.grid.i === i) || (j >= 0 && node.grid.j === j))
    })
  }
  ...
  _updateNodeRectPosition(rootNode) {
    this._selectNodeRectByPath(rootNode.path).attr('x', rootNode.x).attr('y', rootNode.y)
  ...
  _updateGridLine(d, xy, i) {
    this._selectGridLine(xy, i).attr(`${xy}1`, d.position).attr(`${xy}2`, d.position)
  ...
  _setGridHandler(xy) {
    ...
    const dragStarted = (event, d) => {
      targetRootNodes = this._findTargetRootNodes(...this.selectXY(xy, [d.index, -1], [-1, d.index]))
    }
    ...
    const dragEnded = (event, d) => {
      ...
      for (const rootNode of targetRootNodes) {
        this._moveRootNode(
          ...this.selectXY(xy, [rootNode, d.position - rootNode.x, 0], [rootNode, 0, d.position - rootNode.y])
        )
      }
      this._redrawLinkLines()
    }
    const gridHandles = [this._selectGridHandle(xy, -1), this._selectGridLabel(xy, -1)]
  ...
  _setLinkMouseHandler() {
    ...
    const links = [this._selectTpTpLineByPath(), this._selectSupportTpLineByPath()]

    links.forEach((target) => {
      target.on('click', lineClick).on('mouseover', lineMouseOver).on('mouseout', lineMouseOut)
    })
  }
   ...
   */
  _setSVGZoom() {
    this.zoom = zoom().on('zoom', (event) => this.rootSVGGroupSelection.attr('transform', event.transform))
  ...
  highlight(node, className) {
    if (node.type === 'node') {
      this._selectNodeRectByPath(node.path).classed(className, true).style('fill', null)
    } else {
      this._selectTpCircleByPath(node.path).classed(className, true).style('fill', null)
    }
  }
  ...
  _clearAllSelectReady() {
    this.rootSVGGroupSelection.selectAll('.select-ready').classed('select-ready', false)
  }
  ...
  _setTpMouseHandler() {
    ...
    tps.forEach((target) => {
      target
        ...
        .on('mouseover', (event, d) => tpCircleHighlight(event, d, 'select-ready'))
  ...
  _setNodeMouseHandler() {
    ...
    nodes.forEach((target) => {
      target.on('click', nodeClick).on('mouseover', nodeMouseOver).on('mouseout', nodeMouseOut)
    })
  }
  ...
  _maxWHOnGrid(xy, i) {
    ...
    const rootNodes = this._findTargetRootNodes(...this.selectXY(xy, [i, -1], [-1, i]))
  ...
  _gridFittingXY(xy) {
    ...
    for (let i = 0; i < grids.length - 1; i++) {
      ...
      // move root node
      const nextGridRootNodes = this._findTargetRootNodes(...this.selectXY(xy, [i + 1, -1], [-1, i + 1]))
