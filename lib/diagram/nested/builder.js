...
class NestedDiagramBuilder extends MultilayerDiagramBase {
   ...
   */
  colorOfNode(node) {
    return this.isFamilyAggregated(node) ? null : this.colorTable[this.networkPathOf(node.path)]
  }
  ...
  _makeColorTable() {
    ...
    const colorTable = {}
    const layerNameList = this._wholeNodes().map((node) => this.networkPathOf(node.path))
  ...
  _makeSupportTpLines(supportTpLinks) {
    const updatedLines = this.rootSVGGroupSelection.selectAll('line.support-tp').data(supportTpLinks)
  ...
  _makeTpTpLines(tpTpLinks) {
    const updatedLines = this.rootSVGGroupSelection.selectAll('path.tp-tp').data(tpTpLinks)
