...
class InterTpLink extends ForceSimulationLink {
   ...
   */
  isSkewSlash() {
    return (this.y1 > this.y2 && this.x1 < this.x2) || (this.y1 < this.y2 && this.x1 > this.x2)
  }
  ...
  _isSameSkew(otherLink, skewType) {
    return this._skewType() === otherLink._skewType() && this._skewType() === skewType
  }
  ...
  _isOverlapBackslashPatternA(otherLink) {
    return this._inRangeYLow(otherLink.yMid) && this._inRangeX(otherLink.xMax()) && !this._inRangeX(otherLink.xMin())
  }
  ...
  _isOverlapBackslashPatternB(otherLink) {
    return this._inRangeYHigh(otherLink.yMid) && this._inRangeX(otherLink.xMin()) && !this._inRangeX(otherLink.xMax())
  }
  ...
  _isOverlapBackslash(otherLink) {
    return (
      ...
      (this._isOverlapBackslashPatternA(otherLink) || this._isOverlapBackslashPatternB(otherLink))
    )
  }
  ...
  _isOverlapSlashPatternA(otherLink) {
    return this._inRangeYLow(otherLink.yMid) && this._inRangeX(otherLink.xMin()) && !this._inRangeX(otherLink.xMax())
  }
  ...
  _isOverlapSlashPatternB(otherLink) {
    return this._inRangeYHigh(otherLink.yMid) && this._inRangeX(otherLink.xMax()) && !this._inRangeX(otherLink.xMin())
  }
  ...
  _isOverlapSlash(otherLink) {
    return (
      ...
      (this._isOverlapSlashPatternA(otherLink) || this._isOverlapSlashPatternB(otherLink))
    )
  }
  ...
  isOverlap(otherLink) {
    return (
      ...
      (this._isNearYMid(otherLink) || this._isOverlapBackslash(otherLink) || this._isOverlapSlash(otherLink))
    )
  }
  ...
  _wideCircledCornerPolyline(r) {
    if (this.isSkewSlash()) {
      return this._isIrregularYMid() ? this._irregularSlashPolyline(r) : this._regularSlashPolyline(r)
    } else if (this.isSkewBackslash()) {
      return this._isIrregularYMid() ? this._irregularBackslashPolyline(r) : this._regularBackslashPolyline(r)
    }
    // horizontal
    return this._isIrregularYMid() ? this._irregularHorizontalPolyline(r) : this._regularHorizontalPolyline(r)
  }
  ...
  circledCornerPolyline() {
    ...
    if (this.isSkewVertical()) {
      return [`M${this.xMin()},${this.yMin()}`, `L${this.xMax()},${this.yMax()}`].join(' ')
    }
